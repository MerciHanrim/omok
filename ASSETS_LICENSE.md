...
The repository's code (`index.html`, `style.css`, `script.js`) is licensed
separately under the MIT License ([`LICENSE`](LICENSE)) and does **not** extend
to the assets below.
...
- `assets/board/` — ink-wash (Sansuhwa) and other theme board background images
- `assets/stones/` — Baekja (white porcelain) and Meokbit (ink-toned) stone images
...
(**All Rights Reserved**). Even though the code is released under the MIT
license, the assets are **not** covered by that license.
...
The code and assets are licensed separately. The code is under the permissive
MIT license; the self-created visual assets are kept All Rights Reserved so they
are not swept into the code's license when the project is forked or reused. The
sound assets are third-party (Pixabay) from the start, so they are attributed
separately from the visual assets.
